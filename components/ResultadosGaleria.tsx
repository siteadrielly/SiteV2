      ...
      <div className="max-w-[1220px] mx-auto">
        <Reveal className="max-w-[720px]">
          ...
          <p className="text-ink-soft font-light mt-4 max-w-[58ch]">
            Uma seleção de casos autorizados, apresentada com naturalidade e respeito à individualidade de cada paciente.
          </p>
        </Reveal>
            ...
            {cases.slice(0, 6).map((item) => (
              <article key={item.id} className="group">
                <div className="relative aspect-square overflow-hidden border border-line-light bg-[#E8E0D3]">
                  <Image src={item.image_url} alt={`Antes e depois — ${item.procedure}`} fill sizes="(max-width: 640px) 92vw, (max-width: 1024px) 45vw, 30vw" unoptimized className="object-cover transition duration-700 group-hover:scale-[1.02]" />
                </div>
